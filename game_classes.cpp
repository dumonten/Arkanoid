...
//--------------------------------------------------------------------->>> Block
Arkanoid::Block::Block(sf::Vector2f s, sf::Vector2f p, sf::Color c, BlockState st) 
    : DisplayObject(s, p)
{
    state = st; 
...
void Arkanoid::Blocks::init()
{
    ...
    for(int i = 0; i < nSize/2; i++)
    {
        ...
        {
            Block* block = new Block({100, 100}, {i*100, j*100}, sf::Color::Blue, BlockState::NOT_ATTACKED);
        }
    }
}
...
Arkanoid::GameField::GameField(sf::Vector2f s, sf::Vector2f p) : DisplayObject(s, p)
{
    size = s;
    position = p;
    wField = new sf::RenderWindow(sf::VideoMode((int)s.x, (int)s.y), "Arkanoid Game"); 
    wField->setFramerateLimit(60); 
    wField->setPosition(sf::Vector2i(p)); 
}
...
void Arkanoid::Game::mainCycle()
{
    gameField = new GameField(this, {1000, 800}, {300, 500});
    gameField->blocks.init(); 

    while (gameField->wField->isOpen())
    {
        sf::Event event{};
        while (gameField->wField->pollEvent(event)) 
        {
            switch (event.type) {
                case sf::Event::Closed:
                    gameField->wField->close(); 
                    break;
                default:
                    break;
            }
        }
        gameField->wField->clear();  
        gameField->wField->display();   
    }
}
